...
public enum EnumSound {
    ...
    NOTE_BASS_GUITAR("NOTE_BASS_GUITAR", "BLOCK_NOTE_BLOCK_BASS"),
    NOTE_SNARE_DRUM("NOTE_SNARE_DRUM", "BLOCK_NOTE_BLOCK_BASS"),
    ...
    ENDERDRAGON_DEATH("ENDERDRAGON_DEATH", "ENTITY_ENDER_DRAGON_DEATH"),
    ENDERDRAGON_GROWL("ENDERDRAGON_GROWL", "ENTITY_ENDER_DRAGON_GROWL"),
    ENDERDRAGON_HIT("ENDERDRAGON_HIT", "ENTITY_ENDER_DRAGON_HURT"),
    ENDERDRAGON_WINGS("ENDERDRAGON_WINGS", "ENTITY_ENDER_DRAGON_FLAP"),
    ...
    /**
     * Resolve o Sound tentando o nome legado (1.8) primeiro,
     * depois o moderno. Sem comparação de versão — funciona em qualquer servidor.
     * O resultado fica em cache para evitar repetição de try/catch a cada chamada.
     */
    public Sound getSound() {
        if (resolved) return resolvedSound;

        synchronized (this) {
            if (resolved) return resolvedSound;

            Sound result = tryValueOf(sound1_8);
            if (result == null) result = tryValueOf(soundModern);

            if (result == null) {
                Bukkit.getLogger().warning("[EnumSound] Som não encontrado para " + name()
                        + " (tentativas: " + sound1_8 + ", " + soundModern + ")");
            }
    ...
    public void play(Player player, Location location, float volume, float pitch) {
        ...
        if (sound != null) player.playSound(location, sound, volume, pitch);
    }
